...
private:
	...
	void TeleopPeriodic()
	{
		if(!isWait)
		{
			...
			//manipulator
			manip->arm->moveArm(oi->joyManip->GetRawButton(6), oi->joyManip->GetRawButton(7));
			manip->intakeBall(oi->joyManip->GetRawButton(INTAKE_BUTTON), oi->joyManip->GetRawButton(OUTTAKE_BUTTON), (oi->joyManip->GetThrottle()+1)/2);
			catapult->launchBall();
		...
		manip->camera->toggleCameraPosition(bCameraLatch);
	}
